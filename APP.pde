...
color selectedColor;
float slideY=30;
PImage allBlack,choose;
boolean active;float lineSize=0;
...
void DrawImage(int x, int y, PImage a){
   stroke(0);
    imgx=180;imgy=90;
  image(a,x-70,y-50,imgx,imgy);
}
//select color----------------------------
void mouseReleased(){
 select(row1,col1,red,lightRed,darkRed);select(row1,col2,orange,lightOrange,darkOrange);select(row1,col3,yellow,lightYellow,darkYellow);select(row1,col4,green,lightGreen,darkGreen);select(row1,col5,mutedGreen,leaf,moss);select(row1,col6,blue,lightBlue,darkBlue);select(row1,col7,dustyBlue,sky,ocean);select(row1,col8,purple,lightPurple,darkPurple);select(row1,col9,softGold,paleYellow,softTerracotta);select(row1,col10,skinLight,skinMid,skinDark);//10
 //Image
  selectImage(50,200,allBlack);
}
//all button-----------------------
void button1(){
  rectButton(row1,col1,red,lightRed,darkRed);  rectButton(row1,col2,orange,lightOrange,darkOrange);  rectButton(row1,col3,yellow,lightYellow,darkYellow);  rectButton(row1,col4,green,lightGreen,darkGreen);  rectButton(row1,col5,mutedGreen,leaf,moss);  rectButton(row1,col6,blue,lightBlue,darkBlue);  rectButton(row1,col7,dustyBlue,sky,ocean); rectButton(row1,col8,purple,lightPurple,darkPurple);rectButton(row1,col9,softGold,paleYellow,softTerracotta);rectButton(row1,col10,skinLight,skinMid,skinDark);
  stroke(0);
}
//rectbutton-----------------
