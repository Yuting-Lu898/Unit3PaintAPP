...
boolean allblAct,cleanAct,newAct,loadAct;float lineSize=0;
...
void selectImage(int x,int y, PImage a){
  if(mouseX>x&&mouseX<x+wid-5&&mouseY>y&&mouseY<y+leng+15&&allblAct!=true){
    choose=a;
    allblAct=true;
  }
  else if(mouseX>x&&mouseX<x+wid-5&&mouseY>y&&mouseY<y+leng+15&&allblAct==true){
  stroke(0);
  allblAct=false;
  }
}
...
void mousePressed(){
  ...
  }if(allblAct==true)DrawImage(mouseX,mouseY,choose);
}
